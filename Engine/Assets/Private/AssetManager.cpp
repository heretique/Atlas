...
void AssetManager::registerAssetType(AssetType assetType, std::string assetTypeName, AssetFactoryFunc f)
{
    ...
    {
        Logger::log().warn("Asset type '{}', already registered", assetName(assetType));
        return;
    }
...
AssetPtr AssetManager::addAsset(AssetType type, const std::string& filename, u32 flags)
{
    if (filename == "")
    {
        Logger::log().error("Invalid name: '{}' for added Asset of type '{}'", filename, assetName(type));
        return nullptr;
    }
    ...
    if (it == _registry.end())
    {
        Logger::log().error("Asset type not registered: '{}'", assetName(type));
        return nullptr;
    }
    ...
    Logger::log().info("Adding asset: '{}' of type: '{}'", filename.c_str(), assetName(type));
...
bool AssetManager::loadAsset(AssetPtr asset)
{
    ...
    if (ifs)
    {
        if (!asset->load(ifs))
        {
            Logger::log().warn("Couldn't load asset: '{}' of type: '{}'", asset->filename(), assetName(asset->type()));
        }
        else
        {
            ...
            {
                Logger::log().info("Loaded asset: '{}' of type: '{}'", asset->filename(), assetName(asset->type()));
                return true;
            }
        }
    }
    ...
    {
        Logger::log().error("Couldn't find asset: '{}'", path.c_str());
    }
