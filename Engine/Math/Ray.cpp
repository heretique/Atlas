...
Ray::Ray(float originX, float originY, float originZ, float dirX, float dirY,
         float dirZ) {
...
float Ray::intersects(const Frustum &frustum) const {
  ...
  // If the ray's origin is in the negative half-space of one of the frustum's
  // planes
  // and it does not intersect that same plane, then it does not intersect the
  // frustum.
  ...
  // Otherwise, the intersection distance is the minimum positive intersection
  // distance.
...
void Ray::normalize() {
  ...
  float normalizeFactor =
      1.0f / sqrt(_direction.x * _direction.x + _direction.y * _direction.y +
