...
bool Asset::load(const istream &data) {
  ...
  LOGWARNING("Asset '%s' of type %i: No data loaded (file not found?)",
             _name.c_str(), _type);
...
void Asset::setElemParamF(int elem, int elemIdx, int param, int compIdx,
                          float value) {
...
void Asset::setElemParamStr(int elem, int elemIdx, int param,
                            const char *value) {
  LOGERROR("Invalid elem or param in atlasSetResParamStr");
}

void *Asset::mapStream(int elem, int elemIdx, int stream, bool read,
                       bool write) {
...
void AssetManager::registerAssetType(int assetType,
...
AssetHandle AssetManager::addAsset(Asset *resource) {
  AssetHandle handle = _assets.alloc();
  *(_assets.get(handle)) = resource;
  return handle;
}

AssetHandle AssetManager::addAsset(int type, const std::string &name,
                                   int flags) {
  if (name == "") {
...
Asset *AssetManager::getNextAsset(int type, AssetHandle start) {
  ...
  if (startIndex == INVALID_PACKED_INDEX) {
    for (auto i = startIndex + 1; i < packedAssets.count; ++i) {
      ...
      if (res != nullptr &&
          (type == (int)AssetTypes::Undefined || res->_type == type)) {
        return res;
      }
    }
  }
...
void AssetManager::setAssetsDir(const string &path) { _assetDir = path; }
