...
typedef HandledPackedStorage<Asset *, 4096, 20, 12> AssetStorage;
typedef AssetStorage::Handle AssetHandle;
typedef AssetStorage::PackedStorage AssetPackedArray;
static const AssetHandle INVALID_ASSET_HANDLE{0, 0};
...
public:
  ...
  virtual void setElemParamF(int elem, int elemIdx, int param, int compIdx,
                             float value);
  virtual const char *getElemParamStr(int elem, int elemIdx, int param);
  virtual void setElemParamStr(int elem, int elemIdx, int param,
                               const char *value);
  virtual void *mapStream(int elem, int elemIdx, int stream, bool read,
                          bool write);
...
typedef std::function<Asset *(const std::string &name, int flags)>
    AssetTypeFactoryFunc;
...
public:
  ...
  void registerAssetType(int type, const std::string &typeString,
                         AssetTypeFactoryFunc ff, AssetTypeReleaseFunc rf);
